...
#                                                                              #
# Variability computation for 3 EPIC instruments                               #
#                                                                              #
...
# Default variables
DL=8 ; TW=100 ; GTR=1.0 ; BS=3 ; CPUS=12 ; INST=PN

# Default folders
FOLDER_EV=/mnt/xmmcat/4xmm_products
FOLDER_OUT=/mnt/data/Florent
...
case "$1" in
  ...
  shift; shift ;;
  # Folders
  -e|--event-files)	  FOLDER_EV=${2:-$FOLDER_EV}
  shift; shift ;;
  -o|--out)	  	  FOLDER_OUT=${2:-$FOLDER_OUT}
  shift; shift ;;
...
filtering(){
  ...
  if [ ! -f $FOLDER_OUT/$obs/${INST}_clean.fits ] ; then
    echo "bash $SCRIPTS/filtering_kerr.sh -f $FOLDER_EV -o $obs -i $INST" >> $FOLDER_OUT/process_flt_${DL}_${TW}_${GTR}_${BS}_${INST}
  fi
}
...
variability(){
  ...
  # File names
  pathout=$FOLDER_OUT/$obs
  events_file=$path/$(var CLEAN_FILE_${INST})
  gti_file=$path/$(var GTI_FILE_${INST})
  img_file=$path/$(var IMG_FILE_${INST})
